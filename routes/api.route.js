...
router.get('/v1/merk_kendaraan/', async (req, res, next) => {
  try {
    const get_all = await prisma.$queryRaw
    `SELECT brand FROM merk_kendaraan`
...
router.post('/v1/setting/ganti_password/pemohon/', async (req, res, next) => {
  ...
  try {
    ...
    `SELECT nama_pemohon, no_telepon, password FROM m_pemohon WHERE no_telepon = ${no_telepon}`;
    if(check.length === 0){
      res.status(401).send({
        status: 401,
      ...
      const old_pw_compare = await bcrypt.compare(password_lama, check[0].password);

      if(old_pw_compare == true){
        //change password
        const new_pw_hash = bcrypt.hashSync(password_baru, salt);
        const change_pw = await prisma.$executeRaw
        `UPDATE m_pemohon SET password = ${new_pw_hash} WHERE no_telepon = ${no_telepon}`
...
router.post('/v1/reset_password/', async (req, res, next) => {
  ...
  try {
    //check
    const checkifexist = await prisma.$queryRaw
    `select nama_pemohon, no_telepon from m_pemohon where no_telepon = ${no_telepon} `
    ...
    `UPDATE m_pemohon set password = 0 WHERE no_telepon = ${no_telepon}`;

    //change_rand_password
    const pwd = passwordGenerator.generate(8, { upperCaseAlphabets: false, specialChars: false, lowerCaseAlphabets: true});
    
    //change_into_rand
    const rand_pwd_hash = bcrypt.hashSync(pwd, salt);
    const update = await prisma.$executeRaw
    `UPDATE m_pemohon set password = ${rand_pwd_hash} WHERE no_telepon = ${no_telepon}`
...
router.post('/v1/registrasi/dinas', async (req, res, next) => {
  ...
  try {
    const checkDinas = await prisma.$queryRaw
    `SELECT nama_dinas , email FROM m_dinas WHERE nama_dinas = ${nama_dinas} AND email = ${email} `

    if(checkDinas.length == 0){
    const hash = bcrypt.hashSync(password, salt);

      const dinasRegistrasi = await prisma.$executeRaw
      `INSERT INTO m_dinas (nama_dinas, email, password, role, otorisasi) VALUES ( ${nama_dinas}, ${email}, ${hash}, 'dinas', ${otorisasi}) `
...
router.post('/v1/registrasi', async (req, res, next) => {
  ...
  try {
    const check_KTP = await prisma.$queryRaw
      `SELECT nomor_ktp FROM m_pemohon WHERE nomor_ktp = ${nomor_ktp}`;
      if(check_KTP.length === 1){
        const check_No_HP = await prisma.$queryRaw
        `SELECT no_telepon FROM m_pemohon WHERE no_telepon = ${no_telepon}`
        ...
        if(check_No_HP.length){
          ...
          const hash = bcrypt.hashSync(password, salt);
          const registrasi = await prisma.$executeRaw
          `INSERT INTO m_pemohon (nama_pemohon, status_permohonan, alamat, nomor_ktp, ibu_kandung, no_telepon, email, nop, password, provinsi, kota, kecamatan, kelurahan, role) VALUES (${nama_pemohon}, ${jenis_permohonan}, ${alamat}, ${nomor_ktp}, ${ibu_kandung}, ${no_telepon}, ${email}, ${nop}, ${hash},  ${provinsi}, ${kota}, ${kecamatan}, ${kelurahan}, 'pemohon')`;
...
router.post('/v1/login', async (req, res, next) => {
  ...
  try {
      const login = await prisma.$queryRaw
      `SELECT id, nama_pemohon, alamat, no_telepon, email, password, role FROM m_pemohon WHERE email = ${user} OR no_telepon = ${user} OR nama_pemohon = ${user}`
      ...
      if(login.length === 1){
            ...
            if(pemohon_match === true){
              ...
              const getOTP = await prisma.$queryRaw
              `SELECT no_telepon from m_user_otp WHERE no_telepon = ${login[0].no_telepon}`

              //checkifotpexist
              if(getOTP.length === 0){
                //save otp to db
                const saveOTP = await prisma.$executeRaw
                `INSERT INTO m_user_otp (no_telepon, otp) VALUES (${login[0].no_telepon}, ${otp})`
              }else{
                const saveOTP = await prisma.$executeRaw
                `UPDATE m_user_otp SET otp = ${otp} WHERE no_telepon = ${login[0].no_telepon}`
              }
        ...
        const check_dinas = await prisma.$queryRaw
        `SELECT id, email, password, role, otorisasi FROM m_dinas WHERE email = ${user}`

        if(check_dinas.length === 0){
          const check_bank = await prisma.$queryRaw
          `SELECT id_user_bank, nama, no_telepon, password, role, otorisasi, kode_bank FROM m_user_bank WHERE no_telepon = ${user} OR email = ${user}`


          if(check_bank.length === 1){
          const bank_match = await bcrypt.compare(password, check_bank[0].password);
          if(bank_match == true){
            res.status(200).send({
              status: 200,
          ...
          const dinas_match = await bcrypt.compare(password, check_dinas[0].password);
          if(dinas_match == true){
                res.status(202).send({
                  status: 202,
                  message: `Login berhasil !. anda akan masuk ke halaman dashboard`,
...
router.post('/v1/login/enter_otp', async (req, res, next) => {
  ...
  try {
    ...
    const verifOTP = await prisma.$queryRaw
    `SELECT id_otp, otp, no_telepon FROM m_user_otp WHERE no_telepon = ${no_telepon} AND otp = ${otp}`
    ...
    if(verifOTP.length === 0){
      ...
      const login_data = await prisma.$queryRaw
      `SELECT nama_pemohon, alamat, nomor_ktp, no_telepon, email, role FROM m_pemohon WHERE no_telepon = ${verifOTP[0].no_telepon}`

      const clear_OTP = await prisma.$executeRaw
      `UPDATE m_user_otp SET otp = 0 WHERE no_telepon = ${verifOTP[0].no_telepon}`
...
router.post('/v1/pengajuan_fuelcard/', async (req, res, next) => {
  try {
    const {
      nama_pemohon,nomor_ktp,alamat_ktp,telepon,no_penanggung_jawab,email,status_keterangan,tanggal_approve,nomor_polisi,nama_pemilik,merek_kendaraan,alamat_stnk,jenis_kendaraan,model_kendaraan,tahun_pembuatan ,
      isi_silinder,nomor_rangka,nomor_mesin,warna_kendaraan,bahan_bakar,warna_tnkb,tahun_registrasi,nomor_bpkb,masa_berlaku,
      jumlah_roda,keterangan,foto_depan,foto_samping,foto_stnk,foto_ktp,foto_bukti_bayar_pbb,foto_npwp,foto_nib,provinsi,kota,penerbit
    } = req.body;


    const check_nopol = await prisma.$queryRaw
    `SELECT nomor_polisi FROM data_permohonan WHERE nomor_polisi = ${nomor_polisi}`
    ...
    if(check_nopol.length === 1){
      ...
      const pengajuan = await prisma.$executeRaw
      `INSERT INTO data_permohonan 
      ...
      res.status(200).send({
        status: 200,
        message: 'Pengajuan anda sudah terkirim. Petugas akan melakukan verifikasi data terlebih dahulu.'
      })
...
router.post('/v1/data/pembukaan_rekening/', async (req, res, next) => {
  ...
  try {
        const checkifExist = await prisma.$queryRaw
        `SELECT nopol, nama_sesuai_ktp FROM data_pembukaan_rekening WHERE nopol = ${nopol}`
        ...
        if(checkifExist.length === 1){
          ...
          const insert_data = await prisma.$executeRaw
          `INSERT INTO data_pembukaan_rekening (
            nik, 
            nopol,                  
            kode_bank,            
            nama_sesuai_ktp,         
...
router.get('/v1/data/nasabah/', async (req, res, next) => {
  try {
    const getAll = await prisma.$queryRaw
    `SELECT nik,nopol,kode_bank,nama_sesuai_ktp,nama_ibu_kandung,jenis_kelamin,tempat_lahir,tanggal_lahir,alamat,status_alamat,provinsi,kota,kecamatan,kelurahan,kode_pos,npwp,nomor_hp,email,agama,status_kawin,          
      ...
      frekuensi_setoran,rata_rata_setoran,frekuensi_penarikan,rata_rata_penarikan,tujuan_pembukaan_rekening FROM data_pembukaan_rekening, status`
...
router.post('/v1/data/nasabah/by_nopol', async (req, res, next) => {
  ...
  try {
    const get_by_nik = await prisma.$queryRaw
    `SELECT nik,nopol,kode_bank,nama_sesuai_ktp,nama_ibu_kandung,jenis_kelamin,tempat_lahir,tanggal_lahir,alamat,status_alamat,provinsi,kota,kecamatan,kelurahan,kode_pos,npwp,nomor_hp,email,agama,status_kawin,          
      pendidikan_akhir,pekerjaan,tanggal_mulai_kerja,nomor_induk_pegawai,bidang_usaha,jabatan,nama_perusahaan,lokasi_usaha,provinsi_usaha,kota_kerja,penghasilan_perbulan,sumber_penghasilan,penghasilan_tambahan,  
...
router.post('/v1/verifikasi/data/nasabah/', async (req, res, next) => {
  const { nopol, status, waktu_kedatangan, alamat_pengambilan } = req.body;
  try {
    const get_by_nik = await prisma.$queryRaw
    `SELECT nik,nopol,kode_bank,nama_sesuai_ktp,nama_ibu_kandung,jenis_kelamin,tempat_lahir,tanggal_lahir,alamat,status_alamat,provinsi,kota,kecamatan,kelurahan,kode_pos,npwp,nomor_hp,email,agama,status_kawin,          
      pendidikan_akhir,pekerjaan,tanggal_mulai_kerja,nomor_induk_pegawai,bidang_usaha,jabatan,nama_perusahaan,lokasi_usaha,provinsi_usaha,kota_kerja,penghasilan_perbulan,sumber_penghasilan,penghasilan_tambahan,  
    ...
    if(get_by_nik.length === 0){
      ...
      const update_status = await prisma.$executeRaw
      `UPDATE data_pembukaan_rekening SET status = ${status}, waktu_kedatangan = ${waktu_kedatangan}, alamat_pengambilan = ${alamat_pengambilan} WHERE nopol = ${nopol}`
      ...
      if(update_status === 0){
        ...
        if(status === 1){
          res.status(200).send({
            ...
            res.status(200).send({
              status: 200,
              message: `Pengajuan rekening berhasil di Reject`
            })
        }
      }
    }
...
router.post('/v1/data/pengajuan_fuelcard/by_pemohon', async (req, res, next) => {
  try {
    const {nama_pemohon} = req.body;
    const getby_pemohon = await prisma.$queryRaw
    `SELECT id as id_pendaftaran, nama_pemohon, alamat_ktp, telepon, email, status_permohonan, nomor_polisi, nama_pemilik, merek_kendaraan, alamat_stnk, jenis_kendaraan, model_kendaraan, tahun_pembuatan, isi_silinder, nomor_rangka, nomor_mesin, warna_kendaraan, bahan_bakar, warna_tnkb, tahun_registrasi, nomor_bpkb, masa_berlaku, jumlah_roda, keterangan, foto_depan, foto_samping, foto_stnk, foto_ktp, foto_nib, provinsi, kota, penerbit, foto_surat_pernyataan, foto_bukti_bayar_pbb, nomor_telepon_penanggung_jawab, status_keterangan, tanggal_approve, foto_npwp, nomor_ktp, tanggal_pengajuan, kode_bank FROM data_permohonan WHERE nama_pemohon = ${nama_pemohon}`;
...
router.post('/v1/data/pengajuan_fuelcard/by_status', async (req, res, next) => {
  try {
    ...
    const getby_status = await prisma.$queryRaw
...
router.post('/v1/data/pengajuan_fuelcard/by_id', async (req, res, next) => {
  try {
    ...
    const getby_id = await prisma.$queryRaw
    ...
    `select data_permohonan.id as id_pendaftaran, data_permohonan.nama_pemohon , data_permohonan.alamat_ktp , m_pemohon.provinsi, m_pemohon.kota, m_pemohon.kecamatan, m_pemohon.kelurahan, data_permohonan.telepon , data_permohonan.email , data_permohonan.status_permohonan , data_permohonan.nomor_polisi , data_permohonan.nama_pemilik , data_permohonan.merek_kendaraan , data_permohonan.alamat_stnk , data_permohonan.jenis_kendaraan , data_permohonan.model_kendaraan , data_permohonan.tahun_pembuatan , data_permohonan.isi_silinder , data_permohonan.nomor_rangka , data_permohonan.nomor_mesin , data_permohonan.warna_kendaraan , data_permohonan.bahan_bakar , data_permohonan.bahan_bakar , data_permohonan.warna_tnkb , data_permohonan.tahun_registrasi , data_permohonan.nomor_bpkb , data_permohonan.masa_berlaku , data_permohonan.jumlah_roda , data_permohonan.keterangan , data_permohonan.foto_depan , data_permohonan.foto_samping , data_permohonan.foto_stnk , data_permohonan.foto_ktp , data_permohonan.foto_nib , data_permohonan.penerbit , data_permohonan.foto_surat_pernyataan , data_permohonan.foto_bukti_bayar_pbb , data_permohonan.nomor_telepon_penanggung_jawab , data_permohonan.status_keterangan , data_permohonan.tanggal_approve , data_permohonan.foto_npwp , data_permohonan.nomor_ktp , data_permohonan.tanggal_pengajuan , data_permohonan.kode_bank from data_permohonan inner join m_pemohon on m_pemohon.nomor_ktp = data_permohonan.nomor_ktp where data_permohonan.id = ${id} ;`
      ...
      const get_nop = await prisma.$queryRaw
      `SELECT nop FROM m_pemohon WHERE nama_pemohon = ${getby_id[0].nama_pemohon}`
      res.status(200).send({
        status: 200,
        message: `Success`,
        data: getby_id,
...
router.post('/v1/verifikasi/pengajuan_fuelcard/', async (req, res, next) => {
  try {
    const {nama_pemohon, nomor_polisi, status_permohonan, bank_penerbit} = req.body;
    
    const Querry = await prisma.$queryRaw
    `SELECT id, nama_pemohon, nomor_polisi, nama_pemilik, status_permohonan FROM data_permohonan WHERE nama_pemohon = ${nama_pemohon} AND nomor_polisi = ${nomor_polisi}`
    ...
    if(Querry.length == 0){
      ...
      const Update_status = await prisma.$executeRaw
      `UPDATE data_permohonan SET status_permohonan = 1, penerbit = ${bank_penerbit} , kode_bank = ${bank_penerbit} WHERE id = ${Querry[0].id}`

      const succes_update = await prisma.$queryRaw
      `SELECT nama_pemohon, telepon, nama_pemilik, nomor_polisi, nomor_mesin, nomor_rangka , penerbit , status_permohonan FROM data_permohonan WHERE id = ${Querry[0].id}`
      ...
      const data = {
        ...
        message: `Selamat ! Pengajuan Fuel Card anda sudah kami terima.  Kepada Yth. ${nama_pemohon}, pengajuan fuel card dengan nopol ${succes_update[0].nomor_polisi} sudah kami verifikasi. Mohon tunggu informasi selanjutnya mengenai penerbitan kartu. Terima Kasih`
      };
      const customHeaders = {
          "Content-Type": "application/json",
      }
      ...
      // if(succes_update[0].penerbit === '117'){
      //   const url = 'https://app.whacenter.com/api/send'
      //     const data = {
      //       device_id: "3cd45d7ea6f0b8a856c3627004cbf92c",
      //       number: `${succes_update[0].telepon}`,
      //       message: `Selamat ! Pengajuan Fuel Card anda sudah kami terima.  Kepada Yth. ${nama_pemohon}, pengajuan fuel card dengan nopol ${succes_update[0].nomor_polisi} sudah kami verifikasi. Silahkan kunjungi kantor cabang bank sumut di Jl. T. Umar Kompleks Sulaiman Blok A No. 4 - 5 Batam, Kepulauan Riau. Nomor Telepon 0778-452271`
      //     };
      //     const customHeaders = {
      //         "Content-Type": "application/json",
      //     }
      ...
      // }else if(succes_update[0].penerbit === '441'){
      //   const url = 'https://app.whacenter.com/api/send'
      //     const data = {
      //       device_id: "3cd45d7ea6f0b8a856c3627004cbf92c",
      //       number: `${succes_update[0].telepon}`,
      //       message: `Selamat ! Pengajuan Fuel Card anda sudah kami terima.  Kepada Yth. ${nama_pemohon}, pengajuan fuel card dengan nopol ${succes_update[0].nomor_polisi} sudah kami verifikasi. Silahkan kunjungi kantor cabang bank bukopin di alamat : `
      //     };
      //     const customHeaders = {
      //         "Content-Type": "application/json",
      //     }
      ...
      //       device_id: "3cd45d7ea6f0b8a856c3627004cbf92c",
      //       number: `${succes_update[0].telepon}`,
      //       message: `Selamat ! Pengajuan Fuel Card anda sudah kami terima.  Kepada Yth.${nama_pemohon}, pengajuan fuel card dengan nopol ${succes_update[0].nomor_polisi} sudah kami verifikasi. Silahkan kunjungi kantor cabang bank cimb niaga di alamat : `
      //     };
      //     const customHeaders = {
      //         "Content-Type": "application/json",
      //     }
      ...
      const Update_tidak_valid = await prisma.$executeRaw
      `UPDATE data_permohonan SET status_permohonan = 2 WHERE id = ${Querry[0].id}`

      const succes_update_2 = await prisma.$queryRaw
      `SELECT nama_pemohon, nama_pemilik, nomor_polisi, nomor_mesin, nomor_rangka , status_permohonan FROM data_permohonan WHERE id = ${Querry[0].id}`
...
router.post('/v1/history/data/pengajuan/by_pemohon', async (req, res, next) => {
  ...
  try {
    const get_by_pemohon = await prisma.$queryRaw
    `SELECT * FROM data_permohonan WHERE email = ${email} AND nama_pemohon = ${nama_pemohon}`
...
router.post('/v1/alokasi_bbm', async (req, res, next) => {
  const {nama_pemohon, nomor_polisi, jumlah_alokasi, bank_penerbit} = req.body;

  try {
    const check = await prisma.$queryRaw
    `SELECT nomor_polisi, nama_pemohon, jumlah_alokasi FROM alokasi_bbm WHERE nomor_polisi = ${nomor_polisi}`
    if(check.length == 1){
      res.status(400).send({
        status:400,
      ...
      const alokasi = await prisma.$executeRaw
      `INSERT INTO alokasi_bbm (nama_pemohon, nomor_polisi, jumlah_alokasi) VALUES (${nama_pemohon},${nomor_polisi},${jumlah_alokasi})`
...
router.get('/v1/pengajuan/surat_elektronik/:nomor_polisi', async (req, res, next) => {
  const nomor_polisi = req.params.nomor_polisi;

  
  try {
    const get_data = await prisma.$queryRaw
    `SELECT * FROM data_permohonan WHERE nomor_polisi = ${nomor_polisi}`
    
    
    const checker = await prisma.$queryRaw
    `SELECT nomor_polisi,kop_surat FROM surat_elektronik WHERE nomor_polisi = ${nomor_polisi}`
    ...
    if(get_data.length == 0){
      res.status(404).send({
        ...
        var kop_surat
        const get_kuota = await prisma.$queryRaw
        `SELECT * FROM alokasi_bbm WHERE nomor_polisi = ${get_data[0].nomor_polisi}`
        // console.log(get_data[0].nomor_polisi);


        const generate = await prisma.$executeRaw
        `INSERT INTO surat_elektronik (kop_surat, nama_pemohon, nomor_polisi, merk_kendaraan, tipe_kendaraan, kuota_bbm) VALUES 
...
router.get('/v1/data/surat_elektronik/:nomor_polisi', async (req, res, next) => {
  const nomor_polisi = req.params.nomor_polisi;
  
  try {
    const querry = await prisma.$queryRaw
    `SELECT * FROM surat_elektronik WHERE nomor_polisi = ${nomor_polisi}`
    if(querry.length == 1){
      res.status(200).send({
        status:200,
        message: `Success !`,
...
router.get('/v1/dummy/check_nop/:nop', async (req, res, next) => {
  ...
  try {
    const check_nop = await prisma.$queryRaw
    `SELECT * FROM dummy_nop WHERE nop = ${nop}`
...
router.get('/v1/bank/get_all_approved/approved_bank/', async (req, res, next) => {
  try {
    const getdata = await prisma.$queryRaw
...
router.post('/v1/bank/data_approved/input_card/', async (req, res, next) => {
  ...
  try {
    ...
    if(getdata.length === 0){
      ...
      const update_status = await prisma.$executeRaw
      `UPDATE data_pembukaan_rekening SET tanggal_terbit = ${curdate}, status_terbit = 1 WHERE nopol = ${nomor_polisi};`
      
      const insert_nomor_kartu = await prisma.$executeRaw
      `UPDATE alokasi_bbm SET nomor_kartu = ${nomor_kartu}, nomor_rekening = ${nomor_rekening} , status_pembuatan_rekening = 1 , tgl_pembuatan_rekening = ${curdate} WHERE nomor_polisi = ${nomor_polisi};`
...
router.get('/v1/bank/get_all_approved/pemohon/by_bank/:kode_bank', async (req, res, next) => {
...
router.post('/v1/bank/register/', async (req, res, next) => {
  ...
  `SELECT nama, no_telepon FROM m_user_bank WHERE nama = ${nama} OR no_telepon = ${no_telepon}`
  ...
  if(query.length === 1){
    ...
    const hash = bcrypt.hashSync(password, salt);
    const register = await prisma.$executeRaw
    `INSERT into m_user_bank (nama, no_telepon, email, password, role, otorisasi, kode_bank ) VALUES (${nama}, ${no_telepon}, ${email}, ${hash}, 'bank', ${otorisasi}, ${kode_bank} )`
...
router.post('/v1/verifikasi/pengajuan/reject', async (req, res, next) => {
  ...
  try {
    const get_number = await prisma.$queryRaw
    `SELECT nama_pemohon, telepon FROM data_permohonan WHERE nomor_polisi = ${nomor_polisi} `

    const back_level = await prisma.$executeRaw
    `UPDATE data_permohonan SET status_permohonan = 2, status_keterangan = ${alasan_reject} WHERE nomor_polisi = ${nomor_polisi}`
    ...
    if(back_level == 1){
      ...
      const data = {
        ...
        message: `Kepada Yth. Bapak/Ibu *${get_number[0].nama_pemohon}*, Pengajuan Fuel Card anda dengan Nomor Polisi ${nomor_polisi} ditunda dengan alasan *${alasan_reject}*. Harap periksa kembali data pengajuan anda. Terima Kaish`
      };
      const customHeaders = {
          "Content-Type": "application/json",
      }
...
router.post('/v1/update/pengajuan/', async (req, res, next) => {
  const { nomor_polisi,nama_pemilik,merek_kendaraan,alamat_stnk,jenis_kendaraan,model_kendaraan,tahun_pembuatan,
    isi_silinder,nomor_rangka,nomor_mesin,warna_kendaraan,bahan_bakar,warna_tnkb,tahun_registrasi,nomor_bpkb,masa_berlaku,
    jumlah_roda,foto_depan,foto_samping,foto_stnk,foto_ktp,foto_bukti_bayar_pbb,foto_npwp,foto_nib} = req.body;
  

  try {
    const update = await prisma.$executeRaw
    `UPDATE data_permohonan SET status_permohonan = 0, nomor_polisi = ${nomor_polisi}, nama_pemilik = ${nama_pemilik}, merek_kendaraan = ${merek_kendaraan}, alamat_stnk = ${alamat_stnk}, jenis_kendaraan = ${jenis_kendaraan}, model_kendaraan = ${model_kendaraan}, tahun_pembuatan = ${tahun_pembuatan}, 
...
router.post('/v1/setting/running_text/create', async (req, res, next) => {
  ...
  try {
    const create = await prisma.$executeRaw
    `INSERT INTO running_text ( isi_text ) VALUES ( ${isi_text} );`
...
router.get('/v1/setting/running_text/get', async (req, res, next) => {
  try {
    ...
    `SELECT isi_text FROM running_text`
...
router.get('/v1/setting/running_text/get_by/:id', async (req, res, next) => {
  ...
  try {
    const get = await prisma.$queryRaw
    `SELECT isi_text FROM running_text WHERE id = ${id}`
...
router.post('/v1/setting/running_text/edit', async (req, res, next) => {
  ...
  try {
    ...
    `SELECT isi_text FROM running_text`
    if(get.length === 1){
      const edit = await prisma.$executeRaw
      `UPDATE running_text SET isi_text = ${isi_text}`
      if(edit === 1){
        res.status(200).send({
          status: 200,
...
router.post('/v1/setting/berita/create', async (req, res, next) => {
  ...
  try {
    const create = await prisma.$executeRaw
    `INSERT INTO berita (judul, gambar, isi) VALUES (${judul}, ${gambar}, ${isi});`
...
router.get('/v1/setting/berita/get', async (req, res, next) => {
  try {
    const get = await prisma.$queryRaw
    `SELECT id_berita, judul, gambar, tanggal_publish, isi FROM berita`
    var data = [];
...
router.get('/v1/setting/berita/get_by/:id', async (req, res, next) => {
  ...
  try {
    const get = await prisma.$queryRaw
    `SELECT judul, gambar, tanggal_publish, isi FROM berita WHERE id_berita = ${id}`
...
//Edit Berita by id
router.post('/v1/setting/berita/edit', async (req, res, next) => {3
  const { id_berita, judul, gambar, isi } = req.body;
  try {
    const get = await prisma.$queryRaw
    `SELECT id_berita FROM berita WHERE id_berita = ${id_berita}`

    if(get.length === 1){
      const update = await prisma.$executeRaw
      `UPDATE berita SET judul =${judul}, gambar = ${gambar}, isi = ${isi} WHERE id_berita = ${id_berita}`
...
router.post('/v1/setting/berita/delete', async (req, res, next) => {
  ...
  try {
    const get = await prisma.$queryRaw
    `SELECT id_berita , judul FROM berita WHERE id_berita = ${id_berita}`
    if(get.length === 1){
      const hapus = await prisma.$executeRaw
...
//GET DATA UNTUK BANK
router.get('/v1/data/calon_nasabah/approved_by_dinas/', async (req, res, next) => {
  try {
    const get_data = await prisma.$queryRaw
    `select data_permohonan.id as id_pendaftaran, data_permohonan.nomor_polisi, alokasi_bbm.jumlah_alokasi, data_pembukaan_rekening.kode_bank, data_pembukaan_rekening.nama_sesuai_ktp, data_pembukaan_rekening.nama_ibu_kandung, data_pembukaan_rekening.jenis_kelamin, data_pembukaan_rekening.tempat_lahir, data_pembukaan_rekening.tanggal_lahir, data_pembukaan_rekening.alamat, data_pembukaan_rekening.status_alamat, data_pembukaan_rekening.provinsi, data_pembukaan_rekening.kota, data_pembukaan_rekening.kecamatan, data_pembukaan_rekening.kelurahan, data_pembukaan_rekening.kode_pos, data_pembukaan_rekening.npwp, data_pembukaan_rekening.nomor_hp, data_pembukaan_rekening.email, data_pembukaan_rekening.agama, data_pembukaan_rekening.status_kawin, data_pembukaan_rekening.pendidikan_akhir, data_pembukaan_rekening.pekerjaan, data_pembukaan_rekening.tanggal_mulai_kerja, data_pembukaan_rekening.nomor_induk_pegawai, data_pembukaan_rekening.bidang_usaha, data_pembukaan_rekening.jabatan, data_pembukaan_rekening.nama_perusahaan, data_pembukaan_rekening.lokasi_usaha, data_pembukaan_rekening.provinsi_usaha, data_pembukaan_rekening.kota_kerja, data_pembukaan_rekening.penghasilan_perbulan, data_pembukaan_rekening.sumber_penghasilan, data_pembukaan_rekening.penghasilan_tambahan, data_pembukaan_rekening.frekuensi_setoran, data_pembukaan_rekening.rata_rata_setoran, data_pembukaan_rekening.frekuensi_penarikan, data_pembukaan_rekening.rata_rata_penarikan, data_pembukaan_rekening.tujuan_pembukaan_rekening from data_pembukaan_rekening inner join data_permohonan on data_pembukaan_rekening.nik = data_permohonan.nomor_ktp inner join alokasi_bbm on alokasi_bbm.nomor_polisi = data_permohonan.nomor_polisi;`
...
router.post('/v1/data/provinsi/', async (req, res, next) => {
  ...
  try {
    ...
    `INSERT INTO provinsi (nama_provinsi) VALUES (${nama});`
...
router.post('/v1/data/provinsi/kota/', async (req, res, next) => {
  ...
  try {
    const insert = await prisma.$executeRaw
    `INSERT INTO kota_kabupaten (provinsi_id, nama) VALUES ( ${provinsi_id}, ${nama} )`
...
router.post('/v1/data/provinsi/kecamatan/', async (req, res, next) => {
  ...
  try {
    const insert = await prisma.$executeRaw
    `INSERT INTO kecamatan (regensi_id, nama) VALUES ( ${regensi_id}, ${nama} )`
...
router.post('/v1/data/provinsi/kelurahan/', async (req, res, next) => {
  ...
  try {
    const insert = await prisma.$executeRaw
    `INSERT INTO kelurahan (distrik_id, nama_kelurahan, id_kelurahan) VALUES ( ${distrik_id}, ${nama_kelurahan}, ${id_kelurahan} )`
...
router.get('/v1/data/provinsi/', async (req, res, next) => {
  try {
    const querry = await prisma.$queryRaw
    `SELECT id, id_provinsi, nama_provinsi FROM provinsi;`
...
router.get('/v1/data/provinsi/kota/:id_provinsi', async (req, res, next) => {
  ...
  try {
    const querry = await prisma.$queryRaw
    `SELECT provinsi_id, nama, id_kkb FROM kota_kabupaten WHERE provinsi_id = ${id_provinsi} ORDER BY id_kkb ASC;`

    const nama_provinsi = await prisma.$queryRaw
    `SELECT nama_provinsi FROM provinsi WHERE id_provinsi = ${id_provinsi};`
...
//get all kecamatan
router.get('/v1/data/provinsi/kota/kecamatan/:regensi_id', async (req, res, next) => {
  const regensi_id = req.params.regensi_id;
  try {

    const querry = await prisma.$queryRaw
    `SELECT regensi_id, nama, id_kecamatan FROM kecamatan WHERE regensi_id = ${regensi_id} ORDER BY id_kecamatan ASC;`
    ...
    if(querry.length == 0){
      ...
      const nama_kota = await prisma.$queryRaw
      `SELECT nama FROM kota_kabupaten WHERE id_kkb = ${regensi_id}`
...
//get all kelurahan
router.get('/v1/data/provinsi/kota/kecamatan/kelurahan/:distrik_id', async (req, res, next) => {
  const distrik_id = req.params.distrik_id;
  try {
    const querry = await prisma.$queryRaw
    `SELECT nama_kelurahan, distrik_id, id_kelurahan FROM kelurahan WHERE distrik_id = ${distrik_id} ORDER BY id_kelurahan ASC ;`
    ...
    if(querry.length == 0 ){
      ...
      const nama_kecamatan = await prisma.$queryRaw
      `SELECT nama FROM kecamatan WHERE id_kecamatan = ${distrik_id}`
...
router.get('/v1/data/dashboard/', async (req, res, next) => {
  try {
    ...
    //count total pemohon
    const total_pemohon = await prisma.$queryRaw
    `SELECT count(*) as total_pemohon from m_pemohon`

    //count jumlah mobil
    const total_kendaraan = await prisma.$queryRaw
    `select count(*) as total_kendaraan from data_permohonan`

    //total pengajuan sukses
    const pengajuan_sukses = await prisma.$queryRaw
    `select count(*) as total_pengajuan_sukses from data_permohonan where status_permohonan = 1;`

    //total pengajuan gagal
    const pengajuan_gagal = await prisma.$queryRaw
    `select count(*) as total_pengajuan_gagal from data_permohonan where status_permohonan = 3;`

    //total pengajuan pending 
    const pengajuan_pending = await prisma.$queryRaw
    `select count(*) as total_pengajuan_pending from data_permohonan where status_permohonan = 0;`

    //get total liter
    const get_total_liter = await prisma.$queryRaw
    `select sum(jumlah_alokasi) from alokasi_bbm`
    ...
    res.status(200).send({
      ...
      total_pengajuan_sukses: String(pengajuan_sukses[0].total_pengajuan_sukses),
      total_pengajuan_gagal: String(pengajuan_gagal[0].total_pengajuan_gagal),
      total_pengajuan_pending: String(pengajuan_pending[0].total_pengajuan_pending),
